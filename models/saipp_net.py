"""
SAIPP-Net (RadioNet) — UNet-based radio map predictor.

Trained on the IRT4 subset of RadioMapSeer. Given a (building_map, tx_location)
input pair, predicts the received-power radio map.

Reference:
    B. Feng, M. Zheng, W. Liang, and L. Zhang, "SAIPP-Net: A Sampling-Assisted
    Indoor Pathloss Prediction Method for Wireless Communication Systems,"
    IEEE MLSP, 2025.

Note:
    Only the RadioNet class (256→256, 2-input channels) is included here, as it
    is the variant used throughout this work. The original codebase contains
    additional variants (64to32, 128to32, etc.) for different input resolutions.
"""
...
maxpool = nn.MaxPool2d(2, stride=2, padding=0, dilation=1,
                       return_indices=False, ceil_mode=False)


class SDUblock(nn.Module):
    """Stacked Dilated Unit — multi-scale feature extraction via parallel
...
class RadioNet(nn.Module):
    """SAIPP-Net radio map predictor.

    Input:  [B, 2, 256, 256] — channel 0 = building map, channel 1 = Tx one-hot
    Output: [B, 1, 256, 256] — predicted received-power map in [0, 1]
    """
